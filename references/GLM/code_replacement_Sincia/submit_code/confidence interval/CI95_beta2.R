...
AlgTwoStp <- function(r1=r1, r2=r2) {
  if (r2 == 0) {
    ...
    hatpsidot <- exp(x.simp%*%fit.simp$par)
    J_Xsimp <- 1/(n*(r1+r2))*t(x.simp)%*%diag(c(hatpsidot*pinv.simp))%*%x.simp
    V_simp <- 1/(n^2*(r1+r2)^2)*t(x.simp)%*%diag(c((y.simp-hatpsidot)^2*(pinv.simp^2)))%*%x.simp
  ...
  if (r2 != 0) {
    ...
    hatpsidot <- exp(x.mVc%*%fit.mVc$par)
    J_Xmvc <- 1/(n*(r1+r2))*t(x.mVc)%*%diag(c(hatpsidot*pinv.mVc))%*%x.mVc
    V_mvc <- 1/(n^2*(r1+r2)^2)*t(x.mVc)%*%diag(c((y.mVc-hatpsidot)^2*(pinv.mVc^2)))%*%x.mVc
    ...
    hatpsidot <- exp(x.mMSE%*%fit.mMSE$par)
    J_XmMSE <- 1/(n*(r1+r2))*t(x.mMSE)%*%diag(c(hatpsidot*pinv.mMSE))%*%x.mMSE
    V_mMSE <- 1/(n^2*(r1+r2)^2)*t(x.mMSE)%*%diag(c((y.mMSE-hatpsidot)^2*(pinv.mMSE^2)))%*%x.mMSE
