...
samp.data=function(data,beta,num.covariate,num.datalines,num.samp,delta)
{
  ...
  for(i in 1:(dim-1))
  {
    index <- getIdx(use_column, x.matrix[,i], dim.samp, num.samp)
...
full.regression=function(iter,num.covariate,num.datalines,num.samp,delta,initial,dist)
{
  ...
  while(i <= iter)
  {
    ...
    n1<- sum(y)
    n0 <- length(y)-sum(y)
    weight_initial <- rep(1/n0/2,n0+n1)
    weight_initial[y==1]<- 1/n1/2
...
set.seed(10)
num.covariate=c(9)
num.datalines=c(200000, 400000, 600000, 800000)
initial=1000
...
for(i in 1:length(num.datalines)){
  for(j in 1:length(dist))
  {
    a=full.regression(iter,num.covariate,num.datalines[i],num.samp,delta[j],initial,dist[j])
...
result <- data.frame(delta = delta,num.datalines = kronecker(num.datalines,rep(1,4)),num.samp=num.samp,initial=initial,
...
setwd("E:/DESKTOP/big data/diiferent sample size efficiency improvement")
write.csv(result,file = "result04_05_2018 large sample size different data size.txt")


library(ggplot2)

result <- rbind(result,result,result,result)
result$MSE <- result$MSE_lev
result$MSE[33:48] <- result $ MSE_new[1:16]
result$MSE[17:32] <- result $ MSE_srs[1:16]
result$MSE[49:64] <- result $ MSE_full[1:16]
result$Algorithm <- "Leveraging Algorithm (mVc)"
result$Algorithm[33:48] <- "New Algorithm"
result$Algorithm[17:32] <- "Uniform"
result$Algorithm[49:64] <- "FULL"
ggplot(result,aes(x = num.datalines,y = log(MSE),by = Algorithm, color = Algorithm))+
  ...
  labs(x = "Data size n")+ggtitle("Comparison of subsampling algorithms under different distribution")

ggsave("efficiency improve as n larger.png",dpi = 600)
