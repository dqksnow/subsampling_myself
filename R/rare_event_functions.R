...
################################################################################
pilot.estimate <- function(X, Y, n0, criteria = 'optA'){
  N <- nrow(X)
  d <- ncol(X)
  N1 <- sum(Y)
  N0 <- sum(Y == 0)
...
################################################################################
RareLogistic <- function(X, Y, n0, nss, method = 'LogOddsCorrection', criteria = 'optL') {
  N <- nrow(X)
  d <- ncol(X)
  N1 <- sum(Y)
  if (method %in% c("LogOddsCorrection", "Weighted", 'LCC')){
    if (criteria == 'optA'){
      pilot.estimate.results <- pilot.estimate(X, Y, n0, criteria = 'optA')
    } else if (criteria == 'optL'){
      pilot.estimate.results <- pilot.estimate(X, Y, n0, criteria = 'optL')
    }
