...
###############################################################################
poisson.coef.estimate <- function(X,
                                   Y,
                                   offset = NULL,
                                   start = rep(0, ncol(X)),
                                   weights = 1) {
...
random.index <- function (N, n, p) {
  index <- c(sample(N, n, replace = TRUE, prob = p))
  return(index)
}
...
MN <- function (X, lambda, w) {
  MN <- t(X) %*% (X * (lambda * w))
  return(MN)
}
Psi <- function (X, Y, lambda, w) {
  psi <- (Y - lambda)^2
  Psi <- t(X) %*% (X * (psi * w))
  return(Psi)
}
...
pilot.estimate <- function(X, Y, n.plt){
  ...
  # beta.plt <- poisson.coef.estimate(X = x.plt, Y = y.plt)$beta # not corrected yet
  # pbeta.plt <- pbeta(x.plt, beta.plt) # for poisson regression, it represents lambda.plt
  ...
  # P.plt <- pbeta(X, beta.plt) # for poisson regression, it represents lambda.plt
  ...
  p.plt <- p.plt[index.plt]
  # unweighted log likelihood estimation
  beta.plt <- poisson.coef.estimate(X = x.plt, Y = y.plt)$beta
...
subsampling <- function(X, Y, n.ssp, alpha, b, criterion, estimate.method, sampling.method,
  ...
  if (sampling.method == "WithReplacement"){
    ...
    # boxplot(p.ssp)
    # p.ssp <- (1 - alpha) * n.ssp * nm / NPhi + alpha / N
...
subsample.estimate <- function(x.ssp, y.ssp, n.ssp,
  ...
  if (estimate.method == "Weighted") {
    beta.ssp <- poisson.coef.estimate(x.ssp, y.ssp, weights = w.ssp)$beta
    P.ssp <- pbeta(x.ssp, beta.ssp) # for poisson regression, it represents lambda.ssp
    ...
    beta.ssp <- poisson.coef.estimate(X = x.ssp,
                                       Y = y.ssp,
                                       start = beta.plt,
                                       offset = offset)$beta
    P.ssp  <- pbeta(x.ssp, beta.ssp, offset) # for poisson regression, it represents lambda.ssp
...
###############################################################################
poisson.optimal.subsampling <- function(formula, data, n.plt, n.ssp,
                                         criterion = "optL",
  ...
  Y <- model.response(mf, "any") # if we only consider poisson model ,it should not be 'any'.
  ...
  if (estimate.method %in% c("Weighted", "LogOddsCorrection")) {
    ...
    X.uni <- X[index.uni, ]
    Y.uni = Y[index.uni]
    beta.uni <- poisson.coef.estimate(X = X.uni, Y = Y.uni)$beta
