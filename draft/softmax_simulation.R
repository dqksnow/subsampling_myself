...
softmax_simu <- function(j) { #this function is used for parLapply
  ...
  for(i in seq_along(n.ssp)) {
    WithRep.MSPE <- softmax.optimal.subsampling(X, Y, n.plt, n.ssp[i],
                                                criterion = 'MSPE',
                                                sampling.method = 'Poisson',
                                                estimate.method = 'Weighted',
                                                constraint = 'baseline')
    beta.ssp[i, 1, ] <- WithRep.MSPE$beta.ssp
    cov.ssp[i, 1] <- sum(diag(WithRep.MSPE$cov.ssp))
    ...
    WithRep.MSPE.measures <- fitting.measure(beta.pred = WithRep.MSPE$beta.ssp,
                                             beta.true = beta.true.sum,
                                             P.pred = WithRep.MSPE$P.cmb,
                                             P.true = prob.true)
    ...
    cov.ssp[i, 2] <- sum(diag(WithRep.OptA.b$cov.ssp))
    ...
    cov.ssp[i, 3] <- sum(diag(WithRep.OptA.s$cov.ssp))
...
n.plt <- 500
sample.rate <- c(0.05, 0.10, 0.20, 0.30)
n.ssp <- sample.rate*N
...
# plot: compare of empirical mse and theoretical var,
# ssp estimator and full estimator ###########################################
mse_vs_var_plot <- data.frame(rou = n.ssp / N,
                              MSPE.MSE = MeanMSE.ssp[, 1],
                              MSPE.Var.true = Meancov.ssp[, 1],
