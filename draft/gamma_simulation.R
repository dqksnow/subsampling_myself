...
title <- paste('Gamma Regression, MSE, Case 1') # Poisson
...
logistic_simu <- function(j) { #this function is used for parLapply
  ...
  FULL.DATA <- generate.gamma.data(N, case = 1, beta0, shape = 2, seed = j)
  family <- gamma.expand()
  X.full <- FULL.DATA$X
  Y.full <- FULL.DATA$Y
...
mse_vs_2var_plot <- data.frame(rou = n.ssp / N,
                               OptA.poi.MSE = mse.cmb[, 2],
                               OptA.poi.Var.MLE = MeanVar.cmb[, 2],
                               OptA.poi.Var.true = MeanVar.cmb.true[, 2])
