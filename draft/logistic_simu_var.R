...
logistic_simu <- function(j) { #this function is used for parLapply
  ...
  var.ssp <- var.ssp.true <- var.cmb <- var.cmb.true <- SubsampleSize <-
    array(data = NA, dim = c(length(n.ssp), num.method-1))
  ...
  for(i in seq_along(n.ssp)) {
    Wet.OptA <- glm.optimal.subsampling(formula, data, n.ssp[i], n.plt,
                                        criterion = 'OptA',
    ...
    beta.ssp[i, 1, ] <- Wet.OptA$beta.plt
    var.ssp[i, 1] <- sum(diag(Wet.OptA$var.plt))
    ...
    #
    Wet.OptL <- glm.optimal.subsampling(formula, data, n.ssp[i], n.plt,
                                        criterion = 'OptL',
                                        ...
                                        b = 2,
                                        family = family$family.name)
    beta.ssp[i, 2, ] <- Wet.OptL$beta.plt
    var.ssp[i, 2] <- sum(diag(Wet.OptL$var.plt))
    ...
    #
    Wet.LCC <- glm.optimal.subsampling(formula, data, n.ssp[i], n.plt,
                                       criterion = 'LCC',
                                       sampling.method = 'Poisson',
                                       estimate.method = 'Weighted',
                                       alpha = 0,
                                       b = 2,
                                       family = family$family.name)
    beta.ssp[i, 3, ] <- Wet.LCC$beta.plt
    var.ssp[i, 3] <- sum(diag(Wet.LCC$var.plt))
    ...
    #
    Odds.OptA <- glm.optimal.subsampling(formula, data,n.ssp[i], n.plt,
                                         criterion = 'OptA',
                                         sampling.method = 'Poisson',
                                         estimate.method = 'LogOddsCorrection',
                                         b = 2,
                                         family = family$family.name)
    beta.ssp[i, 4, ] <- Odds.OptA$beta.plt
    var.ssp[i, 4] <- sum(diag(Odds.OptA$var.plt))
    ...
    #
    Odds.OptL <- glm.optimal.subsampling(formula, data, n.ssp[i], n.plt,
                                         criterion = 'OptL',
                                         sampling.method = 'Poisson',
                                         estimate.method = 'LogOddsCorrection',
                                         b = 2,
                                         family = family$family.name)
    beta.ssp[i, 5, ] <- Odds.OptL$beta.plt
    var.ssp[i, 5] <- sum(diag(Odds.OptL$var.plt))
    ...
    #
    Odds.LCC <- glm.optimal.subsampling(formula, data, n.ssp[i], n.plt,
                                        criterion = 'LCC',
                                        sampling.method = 'Poisson',
                                        estimate.method = 'LogOddsCorrection',
                                        b = 2,
                                        family = family$family.name)
    beta.ssp[i, 6, ] <- Odds.LCC$beta.plt
    var.ssp[i, 6] <- sum(diag(Odds.LCC$var.plt))
    ...
    #
    Uni <- glm.optimal.subsampling(formula, data, n.ssp[i], n.plt,
                                   estimate.method = 'Uni',
                                   b = 2,
                                   family = family$family.name)
...
mse_vs_2var_plot <- data.frame(rou = n.ssp / N,
                               Wet.OptA = mse.cmb[, 1],
                               Odds.OptA.MSE = mse.ssp[, 1],
                               Odds.OptA.Var.MLE = MeanVar.ssp[, 1])
